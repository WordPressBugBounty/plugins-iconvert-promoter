...
class PopupService {
	...
	public static function create( $data, $promoSettings = array() ) {
		...
		if ( $postID ) {
			...
			$postData = wp_slash(
				array(
					'ID'           => $postID,
					'post_content' => $templateService->importContent( $postContentTemplate, $postID, true , true ),
				)
			);
	...
	public static function getPopupSettings( $post_id ) {
		...
		$animation_in_effect = $animation_in_type === 'none' ? 'none' : $animation_in_effect;
		...
		$animation_out_effect = $animation_out_type === 'none' ? 'none' : $animation_out_effect;
	...
	public static function changeTemplate( $post_id, $template_id ) {
		...
		$postData = wp_slash(
			array(
				'ID'           => $post_id,
				'post_content' => $templateService->importContent( $template->content, $post_id, true, true),
			)
		);
	...
	public static function edit( $postId, $name, $displayConditions, $triggers, $settings = array() ) {
		...
		if ( ! empty( $settings ) ) {
			...
			$content          = $template_service->importContent( wp_slash( $content ), $postId );

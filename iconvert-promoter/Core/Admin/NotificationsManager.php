...
class NotificationsManager {
	...
	private static $remote_data_url_base = 'https://iconvertpromoter.com/wp-json/wp/v2/notification';


	const DISSMISSABLE_NOTICE_OPTION_KEY = '_iconvertpr_dismissible_notices';

	public static function load() {
		add_action( 'admin_init', array( NotificationsManager::class, 'init' ) );
		add_action( 'wp_ajax_iconvertpr-dismissable-notice--dismiss', array( NotificationsManager::class, '_dismiss_dismissable_notice' ) );
	 ...
	 * Relies on the `ICONVERTPR_NOTIFICATIONS_DEV_MODE` constant.
	 *
	 * @return bool
	 */
	private static function isDevMode() {
		return ( defined( 'ICONVERTPR_NOTIFICATIONS_DEV_MODE' ) && ICONVERTPR_NOTIFICATIONS_DEV_MODE );
	}
	...
	public static function init() {
		...
		add_action( 'wp_ajax_iconvertpr-remote-notifications-retrieve', array( NotificationsManager::class, 'updateNotificationsData' ) );
	}
		...
		add_action(
			...
			function () {
				$fetch_url = add_query_arg(
					array(
						'action'   => 'iconvertpr-remote-notifications-retrieve',
						'_wpnonce' => wp_create_nonce( 'iconvertpr-remote-notifications-retrieve-nonce' ),
	...
	public static function updateNotificationsData() {
		check_ajax_referer( 'iconvertpr-remote-notifications-retrieve-nonce' );
		...
		$url = add_query_arg(
			array(
				...
				'meta_value'               => apply_filters( 'iconvertpr/notifications/license_type', 'free' ),
				'iconvertpr_version'       => ICONVERTPR_VERSION,
				'iconvertpr_build'         => ICONVERTPR_BUILD_NUMBER,
				'iconvertpr_theme_version' => wp_get_theme()->get( 'Version' ),
	...
	/**
	 * Adds the stack of notifications for display using `iconvertpr_add_dismissable_notice`.
	 *
	...
	private static function displayNotifications( $notifications ) {
		...
		foreach ( $notifications as $notification ) {
			...
			$classnames    = 'iconvertpr-remote-notification';
			$allowed_types = array( 'info', 'warning', 'error', 'success' );

			if ( ! empty( $params['type'] ) && in_array( $params['type'], $allowed_types ) ) {
				$classnames .= ' notice-' . $params['type'] . ' iconvertpr-remote-notification-' . $params['type'];
			}

			// $notice_key = 'iconvertpr-remote-notice-' . $params['id'];
	...
	public static function displayNotification( $params ) {
		...
		wp_enqueue_style( 'iconvertpr-notifications', ICONVERTPR_URL . 'admin/assets/notifications.css', array(), ICONVERTPR_VERSION );
		?>
		<div class="iconvertpr-remote-notification-wrapper" id="iconvertpr-remote-notification-<?php echo esc_attr( $params['id'] ); ?>">
			<div class="iconvertpr-remote-notification-icon">
				<img src="<?php echo esc_url( ICONVERTPR_URL . 'admin/assets/images/notice-logo.png' ); ?>" alt="<?php esc_attr_e( 'iConvert Promoter', 'iconvert-promoter' ); ?>">
			</div>
			<?php if ( ! empty( $params['message'] ) ) { ?>
				<div class="iconvertpr-remote-notification-message">
				<?php
			...
			<?php } ?>
			<div class="iconvertpr-remote-notification-buttons">
				<?php if ( ! empty( $link ) ) { ?>
					<a target="_blank" href="<?php echo esc_url( $link['url'] ); ?>" class="iconvertpr-remote-notification-primary"><?php echo esc_html( $link['title'] ); ?></a>
					<?php
				}

				if ( ! empty( $slink ) ) {
					?>
					<a target="_blank" href="<?php echo esc_url( $slink['url'] ); ?>" class="iconvertpr-remote-notification-secondary"><?php echo esc_html( $slink['title'] ); ?></a>
				<?php } ?>
			</div>
		</div>
		<?php
	...
	private static function isTimeToDisplay( array $params ) {
		...
		$install_time = apply_filters( 'iconvertpr/notifications/install_time', $install_time );
	...
	private static function getTransientKey() {
		$transient = apply_filters( 'iconvertpr/notifications/transient_key', 'iconvertpr_remote_notifications' );
		return $transient;
	}
	...
	public static function addDismissableNotice( $name, $callback, $repeat_after = 0, $params = array(), $classes = '' ) {
		...
		add_action(
			...
			function () use ( $name, $params, $callback, $classes ) {
				$id   = 'iconvertpr-notice-' . uniqid();
				$data = array(
					'id'   => $id,
					'name' => $name,
				);
				?>
					<div data-iconvertpr-notice-id="<?php echo esc_attr( $id ); ?>" class="notice is-dismissible <?php echo esc_attr( $classes ); ?>">
						<?php call_user_func( $callback, $params ); ?>
					<script>
						jQuery(function($){
								var data =<?php	echo wp_json_encode( $data ); ?>;
							$(document).on('click','[data-iconvertpr-notice-id=' + data.id + '] .notice-dismiss',function(){
								wp.ajax.post('iconvertpr-dismissable-notice--dismiss',{
									iconvertpr_notice_name:data.name,
										_wpnonce: '<?php echo esc_html( wp_create_nonce( 'iconvertpr-dismissable-notice--dismiss-nonce' ) ); ?>'
								});
							});
						});
					</script>
	...
	public static function _dismiss_dismissable_notice() {
		check_ajax_referer( 'iconvertpr-dismissable-notice--dismiss-nonce' );
		$notice  = Arr::get( $_REQUEST, 'iconvertpr_notice_name', false );
		$notices = get_option( NotificationsManager::DISSMISSABLE_NOTICE_OPTION_KEY, array() );

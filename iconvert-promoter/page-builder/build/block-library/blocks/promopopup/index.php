...
class PromoPopupBlock extends BlockContainerBase {
	...
	public function mapPropsToElements() {
		...
		$animationDuration = max( floatval( $animationDuration ), 0.005 );
		...
		return array(
			...
			self::OUTER       => array_merge(
				array(
					...
					'data-show-effect'        => $showEffectClass === 'none' ? 'animate__None' : $showEffectClass,
					'data-exit-effect'        => $hideEffectClass === 'none' ? 'animate__None' : $hideEffectClass,
			...
			self::CONTAINER   => array(
				...
				'data-animation-duration' => strval( $animationDuration ),
				'style'                   => array(
					'animation-duration' => strval( $animationDuration ) . 's',
				),
			),
